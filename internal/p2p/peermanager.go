...
func (m *PeerManager) TryDialNext() (NodeAddress, error) {
	...
	if m.options.MaxConnected > 0 && m.NumConnected()+len(m.dialing) >=
		...
		fmt.Printf("[YIREN-DEBUG] Skip dialing, connected to maximum number of peers %d, dialing: %d\n", m.NumConnected(), len(m.dialing))
		return NodeAddress{}, nil
	}

	for _, peer := range m.store.Ranked() {
		if m.dialing[peer.ID] || m.connected[peer.ID] {
			if m.dialing[peer.ID] {
				fmt.Printf("[YIREN-DEBUG] Skip dialing peer.ID: %s, because it is still dialing\n", peer.ID)
			}
			continue
		}
