...
func (r *Reactor) OnStart(ctx context.Context) error {
	...
	go func() {
		...
		err := scope.Run(ctx, func(ctx context.Context, s scope.Scope) error {
		  ...
		  s.Spawn(func() error { return r.updateRoundStateRoutine(ctx) })
			s.Spawn(func() error { return r.processChannels(ctx) })
			s.Spawn(func() error { return r.processPeerUpdates(ctx, peerUpdates) })
			return nil
		})
		if utils.IgnoreCancel(err)!=nil {
			r.logger.Error("consensus.Reactor", "err", err)
		}
	}()
	return nil
}
...
// upon receiving.
func (r *Reactor) subscribeToBroadcastEvents(ctx context.Context) error {
	onStopCh := r.state.getOnStopCh()

	if err := r.state.evsw.AddListenerForEvent(
		listenerIDConsensus,
		...
		return fmt.Errorf("failed to add listener for events: %w", err)
	}
...
func (r *Reactor) gossipDataForCatchup(ctx context.Context, rs *cstypes.RoundState, prs *cstypes.PeerRoundState, ps *PeerState) {
	...
	if index, ok := prs.ProposalBlockParts.Not().PickRandom(); ok {
		...
		err = r.channels.data.Send(ctx, p2p.Envelope{
			To: ps.peerID,
...
func (r *Reactor) pickSendVote(ctx context.Context, ps *PeerState, votes types.VoteSetReader) (bool, error) {
	...
	if err := r.channels.vote.Send(ctx, p2p.Envelope{
		To: ps.peerID,
		Message: &tmcons.Vote{
			Vote: vote.ToProto(),
		},
...
func (r *Reactor) gossipVotesRoutine(ctx context.Context, ps *PeerState) error {
	...
	for {
		...
		if blockStoreBase > 0 && prs.Height != 0 && rs.Height >= prs.Height+2 && prs.Height >= blockStoreBase {
			...
			if ok, err := r.pickSendVote(ctx, ps, ec); err != nil {
				return fmt.Errorf("r.pickSendVote(): %w", err)
			} else if ok {
...
func (r *Reactor) processPeerUpdate(ctx context.Context, peerUpdate p2p.PeerUpdate) {
	...
	case p2p.PeerStatusDown:
		if ps, ok := r.peers[peerUpdate.NodeID]; ok {
			r.mtx.Lock()
...
// gracefully.
func (r *Reactor) processChannels(ctx context.Context) error {
	return scope.Run(ctx, func(ctx context.Context, s scope.Scope) error {
		for _,ch := range []*p2p.Channel{
			r.channels.state,
			r.channels.data,
			r.channels.vote,
			r.channels.votSet,
			...
			s.Spawn(func() error {
				...
				for iter.Next(ctx) {
					...
					if err := r.handleMessage(ctx, envelope); err != nil {
						...
						if serr := r.channels.state.SendError(ctx, p2p.PeerError{
...
func (r *Reactor) peerStatsRoutine(ctx context.Context, peerUpdates *p2p.PeerUpdates) error {
	for {
