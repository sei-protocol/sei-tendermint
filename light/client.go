...
func (c *Client) AddProvider(p provider.Provider) {
	c.providerMutex.Lock()
	defer c.providerMutex.Unlock()
	c.logger.Info("adding new provider to witnesses", "provider", p.ID(), "witnesses", len(c.witnesses))
...
func (c *Client) RemoveProvider(p provider.Provider) {
	c.providerMutex.Lock()
	defer c.providerMutex.Unlock()
	c.logger.Info("removing provider from witnesses", "provider", p.ID(), "witnesses", len(c.witnesses))
...
func (c *Client) lightBlockFromPrimary(ctx context.Context, height int64) (*types.LightBlock, error) {
	c.logger.Info("[TM-DEBUG] getting light block from primary", "height", height, "primary", c.primary.ID())
	c.providerMutex.Lock()
...
func (c *Client) getLightBlock(ctx context.Context, p provider.Provider, height int64) (*types.LightBlock, error) {
	...
	if ctx.Err() != nil {
		c.logger.Error("light block request context failed", "err", ctx.Err(), "height", height, "primary", p.ID())
		return nil, provider.ErrNoResponse
	}
	if err != nil {
		c.logger.Error("light block request failed with err", "err", err, "height", height, "primary", p.ID())
	}
	return l, err
}
